...
;; Modified by: Renzix (eragon9981@gmail.com/Daniel DeBruno)
...
(defun acme-mouse-kill-word-or-region ()
...
;; ESHELL ADVICE
(defvar acme-mouse-redirection-input nil)
(defun acme-mouse-eshell-strip-redirections (terms)
  ...
  (let ((tl terms)
    ...
    (while tt
	  ...
	  (setq tt (cdr tt)
		tl (cdr tl))
	(unless (cdr tt) ;; @TODO(Renzix): DEFAULT >>> to insert to (current-buffer)
            (setq tt (nconc tt (list (current-buffer)))))
...
;;(advice-add #'eshell-strip-redirections :override #'acme-mouse-eshell-strip-redirections)

(defun acme-mouse-eval ()
  "Evals shell and replaces it."
  (interactive)
  (acme-mouse-kill-word-or-region)
  ;; Parse it???
  (let ((out (eshell-command (car kill-ring) t)))
    (unless acme-mouse-redirection-input
      (insert out))))
...
;; Default to searching for word if no action is found
(defun acme-search-string (string)
  (acme-search t))

(advice-add #'wand-helper:eval-string :override #'acme-search-string)
...
(defun move-mouse-to-point ()
  ...
  (let* ((coords (posn-col-row (posn-at-point)))
	 ...
	 (y (+ (cdr coords) (cadr window-coords)
		   ...
		   -1
		 0))))
    (set-mouse-position (selected-frame) (+ x 1) y)))
