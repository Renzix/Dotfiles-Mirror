...
default or at least some form of it as this doesnt work if you
dont have sudo. This now works with [[https://www.emacswiki.org/emacs/TrampMode#toc17][tramp]] as before it doesnt work
without editing the file path.

#+name: sudo-edit
#+begin_src emacs-lisp

  (defvar sudo-edit-prefix (if (featurep 'xemacs) "/[sudo/root@localhost]" "/sudo:root@localhost:" )
    "*The filename prefix used to open a file with `sudo-edit.")

  (defvar sudo-edit-history nil
    "History list for files found using `sudo-edit'.")

  (defvar sudo-edit-hook nil
    "Normal hook for functions to run after finding a \"root\" file.")

  (defun sudo-edit ()
    "*Open a file as the root user.
           Prepends `sudo-edit-prefix' to the selected file name so that it
    ...
    (let* ( ;; We bind the variable `file-name-history' locally so we can
           ...
           (file-name-history sudo-edit-history)
      ...
      (when (setq file (read-file-name "Find file (UID = 0): " dir path))
        (find-file (concat sudo-edit-prefix file))
        ;; If this all succeeded save our new history list.
        (setq sudo-edit-history file-name-history)
        ;; allow some user customization
        (run-hooks 'sudo-edit-hook))))
#+end_src
