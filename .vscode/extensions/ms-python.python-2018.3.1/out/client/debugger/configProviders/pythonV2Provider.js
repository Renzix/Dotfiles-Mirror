...
let PythonV2DebugConfigurationProvider = class PythonV2DebugConfigurationProvider extends baseProvider_1.BaseConfigurationProvider {
    ...
    provideDefaults(workspaceFolder, debugConfiguration) {
        ...
        if (this.serviceContainer.get(types_1.IPlatformService).isWindows) {
            debugConfiguration.debugOptions.push(Contracts_1.DebugOptions.FixFilePathCase);
        }
        if (debugConfiguration.module && debugConfiguration.module.toUpperCase() === 'FLASK'
            && debugConfiguration.debugOptions.indexOf(Contracts_1.DebugOptions.Jinja) === -1) {
            debugConfiguration.debugOptions.push(Contracts_1.DebugOptions.Jinja);
        }
    }
};
