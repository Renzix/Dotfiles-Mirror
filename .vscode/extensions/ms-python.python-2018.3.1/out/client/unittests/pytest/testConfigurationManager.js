...
class ConfigurationManager extends testConfigurationManager_1.TestConfigurationManager {
    constructor(workspace, outputChannel, installer, testConfigSettingsService) {
        super(workspace, types_1.Product.pytest, outputChannel, installer, testConfigSettingsService);
    }
    ...
    configure(wkspace) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const installed = yield this.installer.isInstalled(types_1.Product.pytest);
            if (!installed) {
                yield this.installer.install(types_1.Product.pytest);
            }
            yield this.testConfigSettingsService.updateTestArgs(wkspace.fsPath, types_1.Product.pytest, args);
        });
    }
}
