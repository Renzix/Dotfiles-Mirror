...
** init.el
   ...
   Including my init.el for documentation purposes. Note that this is
   NOT in my config.org file.

   #+INCLUDE: "~/Dotfiles/.emacs.d/init.el" src emacs-lisp :tangle no
     ...
     #+BEGIN_SRC emacs-lisp
       (setq rcirc-default-nick "Renzix"
             ...
             '(("irc.freenode.net" :channels ("#emacs")))
               rcirc-authinfo
               '(("irc.freenode.net" nickserv "Renzix" "")))
...
*** whole-line-or-region-mode
    ...
    Makes it so M-w and C-w default to the current line instead of a
    invisable point. Really nice cuz C-w is the same as dd now.

    #+NAME: whole-line-or-region-mode
    #+BEGIN_SRC emacs-lisp
...
**** java
     ...
     them. lsp-java, meghanada and eclim. For now eclim is disabled as
     its annoying installing and confiuring it properly.
...
**** web-mode
     ...
     Does all website stuff. For now im only using it for html/htm/css files
...
*** c and cpp
    ...
    it can be installed inside emacs. Uses [[lsp]] to do stuff. THIS IS
    DISABLED CUZ LSP IS AWESOME.
...
**** Helm
     ...
     #+BEGIN_SRC emacs-lisp
       ...
       (general-define-key
        :keymaps '(helm-multi-swoop-map helm-swoop-map)
        "C-s" 'helm-next-line
...
**** Company
     ...
     Arrow keys are annoying so im using C-n and C-p to switch
     selection of company autocompletions.

     @TODO(renzix): Make this work as rn it rebinds C-n and C-p in
     emacs state.

     #+NAME: keybinds-company
     #+BEGIN_SRC emacs-lisp :tangle no 
       (general-define-key
