...
**** golang
     ...
     Lets also add syntax/error checking with goflymake because it
     uses native golang tools unlike flymake-go which only uses gofmt.
     
     #+NAME: goflymake
     #+BEGIN_SRC emacs-lisp
       (use-package goflymake)
     #+END_SRC
...
** Thoughts on keybindings
   ...
   multiple ways to look at a single file depending on context.
