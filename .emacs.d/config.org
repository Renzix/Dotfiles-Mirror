...
** Thoughts on keybindings
   ...
   keymaps. Arguments are achieved through C-u or evil
   motions/1-9/registers. chords are by key-chord or its in vim by
   ...
   the other big thing that emacs does better.
   
   - states (insert or normal)
   - prefix (C-x or evil leader)
   - argument (2dd or C-u)
   - chords (keychord)
   - commands (M-x or ex-commands)
   - modifers (C-y or C-S-<backspace>)
   - context (org mode C-c C-c)
