...
** Linum Relative
   ...
   #+END_SRC
** Evil-Buffer-State
   Attempt at making a new emacs state which i need to finish
   @TODO(renzix): Make this work
   #+BEGIN_SRC emacs-lisp
     ...
     (evil-define-state buffer
       ...
       :tag " <B> "
       :message "-- BUFFER --"
       :entry-hook (evil-buffer-start-hook)
       :exit-hook (evil-buffer-stop-hook)
       (evil-define-key 'buffer global-map 
	 (kbd "q") 'delete-window
	 (kbd "Q") 'kill-emacs
     ...
     (defun evil-buffer-start-hook ()
       ;; Put into window-mode-map
       (message "Entering buffer mode"))

     (defun evil-buffer-stop-hook ()
       ;; Exit window-mode-map
       (message "Exiting buffer mode"))
   #+END_SRC
** Keybindings
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (evil-define-key 'buffer global-map (kbd ";") 'helm-M-x)
     (evil-define-key 'normal global-map (kbd "SPC") 'app-launcher)
     (evil-define-key 'buffer global-map (kbd "ESC") 'normal-mode)
     (evil-define-key 'buffer global-map (kbd ":") 'evil-command-window-mode)
     (evil-define-key 'normal global-map (kbd "\\") 'evil-buffer-state)
...
** Text
   ...
   #+END_SRC
** Discord
   @TODO(renzix): make this work possibly?
   #+BEGIN_SRC emacs-lisp
     ;; For Rich presence
     (use-package elcord)
     (setq elcord-mode t)
