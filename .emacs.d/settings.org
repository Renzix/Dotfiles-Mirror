...
** Init
   ...
   #+END_SRC
** Motion-State
   I originally made a new state called Movement state but then
   relized i can just use Motion state as it has no keybinds anyway
   and inherits most of normal state
   #+BEGIN_SRC emacs-lisp
     (defvar my-evil-prev-state nil "Previous buffer state for motion state on everything")
     (add-hook 'evil-motion-entry-hook 
	       '(lambda ()
		  (setq prev-buffer-state "reee")))
     (evil-define-key '(normal insert) global-map (kbd "`") 'evil-motion-state) ;
     (evil-define-key '(normal insert) global-map (kbd "~") 'evil-motion-state) ;; Make this for a single command uwu
     (evil-define-key 'motion global-map 
       (kbd "ESC") 'evil-change-to-previous-state
       (kbd "`") (lambda () (interactive) (insert 96)) ;; insert `
       (kbd "~") (lambda () (interactive) (insert 126)) ;; insert ~
       (kbd "q") 'delete-window
       (kbd "Q") 'kill-emacs
     ...
     (general-define-key
      :states '(motion)
      :prefix "]" ;;@TODO(renzix): Find a better prefix???
...
** Keybindings
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (which-key-mode)
     (evil-define-key '(normal motion) global-map (kbd ";") 'helm-M-x)
     (evil-define-key '(normal motion) global-map (kbd "SPC") 'app-launcher)
     (evil-define-key '(normal motion) global-map (kbd "gc") 'comment-line)
   #+END_SRC
* Programming
** Projectile
   #+BEGIN_SRC emacs-lisp
