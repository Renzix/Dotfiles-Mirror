...
** Movement-State
   ...
   #+BEGIN_SRC emacs-lisp
     (evil-define-key '(normal insert) global-map (kbd "`") 'evil-execute-in-movement-state) ;
     (evil-define-key '(normal insert) global-map (kbd "~") 'evil-movement-state) ;; Make this for a single command uwu
